...
from project import (
    ...
    run_once,
    safe_load_dotenv,
    get_schemas_sorted_by_category,
...
def calculate_schema_target(schema_id: str, schema_data: dict) -> int:
    """
    Per-schema targets aligned with SchemaQueue (simple_generator_ui).

    M*: 3 or 5 when more than 3 exemplar IDs; P/C/B: 4 + len(exemplar_ids).
    """
    exemplar_ids = schema_data.get("exemplar_ids", [])
    prefix = schema_id[0].upper() if schema_id else ""
    if prefix == "M":
...
class WorkerManager:
    ...
    def __init__(self, base_dir: str, cfg: RunConfig, models: ModelsConfig,
        ...
        if self.systematic_config.mode == "systematic":
            # Load and sort schemas (Schemas_ESAT.md only)
            _path, schemas_md = load_schemas_esat_markdown(base_dir)
