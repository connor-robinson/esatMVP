...
class GenerationControlsWidget(ttk.LabelFrame):
    ...
    def start_generation(self):
        ...
        try:
            # esat_question_generator/ (sibling to schema_generator under question-generation/)
            current_file = Path(__file__)
            esat_pkg = current_file.parents[2]
            script_path = esat_pkg / "generate_with_progress.py"
            project_root = esat_pkg.parent.parent
            ...
            schema_coverage_path = (
                esat_pkg.parent / "schema_generator" / "_cache" / "schema_coverage.json"
            )
            ...
            for schema_id, data in sorted_schemas:
                ...
                if difficulty_data:
                    ...
                    for diff in ["Easy", "Medium", "Hard", "Extreme"]:
                        ...
                        if count > 0:
                            label = "X" if diff == "Extreme" else diff[0]
                            diff_parts.append(f"{label}:{count}")
