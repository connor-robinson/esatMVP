...
from question_review.uploader_url import default_uploader_base_url
...
class QuestionDetailWidget(ttk.Frame):
    ...
    def render_question(self):
        ...
        self._create_media_capture_section()
    ...
    def _create_media_capture_section(self):
        """Upload code + iPad uploader URL; one combined screen+mic video per question."""
        if not self.current_question or not self.db:
            return

        qid = self.current_question["id"]
        section = ttk.LabelFrame(
            self.content_frame,
            text="Walkthrough video (screen + mic, one file)",
            padding=12,
        )
        section.pack(fill=tk.X, padx=10, pady=8)

        try:
            code = self.db.ensure_media_upload_code(qid)
        except Exception as e:
            err = ttk.Label(
                section,
                text=f"Media setup unavailable: {e}\nRun migrations/add_question_media.sql and create bucket 'question-media'.",
                foreground="#b91c1c",
                wraplength=700,
            )
        ...
        code_shell = tk.Frame(section, bg="#fef08a", padx=14, pady=12, highlightbackground="#ca8a04", highlightthickness=2)
        code_shell.pack(fill=tk.X, pady=(0, 10))

        tk.Label(
            code_shell,
            text="UPLOAD CODE (enter on iPad)",
        ...
        ttk.Button(btn_row, text="Copy code", command=copy_code).pack(side=tk.LEFT, padx=(0, 8))

        def refresh_media():
            updated = self.db.get_question_by_id(qid)
            if updated:
                self.load_question(updated)
                if self.on_question_updated:
                    self.on_question_updated()

        ttk.Button(btn_row, text="Refresh from cloud", command=refresh_media).pack(side=tk.LEFT)

        base_url = default_uploader_base_url()
        uploader_line = f"{base_url}/uploader"
        u_frame = ttk.Frame(section)
        u_frame.pack(fill=tk.X, pady=(0, 4))
        ttk.Label(u_frame, text="On iPad, open:").pack(side=tk.LEFT)
        ent = ttk.Entry(u_frame, width=56)
        ent.insert(0, uploader_line)
        ent.config(state="readonly")
        ent.pack(side=tk.LEFT, padx=6, fill=tk.X, expand=True)

        def open_uploader():
            webbrowser.open(uploader_line)

        ttk.Button(u_frame, text="Open", command=open_uploader).pack(side=tk.LEFT)
        ttk.Label(
            section,
            text="Uses this PC’s LAN IP and port 8765 (override with UPLOADER_HOST / UPLOAD_SERVER_PORT if wrong).",
            foreground="gray",
            wraplength=720,
            font=("", 8),
        ).pack(anchor=tk.W, pady=(0, 2))

        screen_path = self.current_question.get("screen_video_storage_path")

        badge_row = ttk.Frame(section)
        badge_row.pack(fill=tk.X, pady=6)
        ttk.Label(badge_row, text="Status:", font=("", 9, "bold")).pack(side=tk.LEFT, padx=(0, 6))

        def pill(text: str, bg: str) -> tk.Label:
            return tk.Label(
                badge_row,
                text=text,
                fg="white",
        ...
        if screen_path:
            pill("Video uploaded", "#15803d").pack(side=tk.LEFT, padx=2)
        else:
            pill("No video yet", "#64748b").pack(side=tk.LEFT, padx=2)

        prev_row = ttk.Frame(section)
        prev_row.pack(fill=tk.X, pady=4)

        def open_signed(path: Optional[str], kind: str):
            if not path:
                messagebox.showinfo("Preview", f"No {kind} file yet.")
                return
            url = self.db.create_signed_media_url(path, expires_in=7200)
            if not url:
                messagebox.showerror("Preview", "Could not create a signed URL (check Storage bucket and path).")
                return
            webbrowser.open(url)
        ...
        ttk.Button(
            prev_row,
            text="Preview video",
            command=lambda: open_signed(screen_path, "video"),
        ).pack(side=tk.LEFT)
