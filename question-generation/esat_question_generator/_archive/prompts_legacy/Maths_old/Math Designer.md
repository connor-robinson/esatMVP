...
- **Hides the solution method** — the question should not immediately reveal the technique needed (e.g., don't say "complete the square" if that's the method; instead, present it in a way that requires the candidate to recognize when to use it)
...
7. **Do NOT explicitly name the solution technique** — if the schema mentions "complete the square", design the idea so that completing the square is the natural approach, but don't state it directly. The question should require the candidate to recognize the technique, not have it handed to them.
...
**CRITICAL**: Do NOT use markdown backticks (`` ` ``) or markdown code blocks (e.g., ` ```yaml `) in your response. Return ONLY the raw YAML string.

```yaml
schema_id: <schema id>
idea_summary: >
  One or two sentences describing the core reasoning the student must perform.
function_or_object_type:
  - polynomial / rational / trigonometric / exponential / geometric / physical system
  - (choose one or two, not specific forms)
constraints_used:
...
intended_wrong_paths:
  - short descriptions of the most likely reasoning mistakes
