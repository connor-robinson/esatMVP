...
**Implementation note (v2):** The live pipeline is implemented in `project.py` (`run_once`: Designer → Implementer → Verifier → Style Judge → KaTeX gate → optional classifier tags → `accepted.jsonl` / DB). Math uses `by_subject_prompts/new/Math1/`; other subjects use `by_subject_prompts/old/<Subject>/` plus shared files in `by_subject_prompts/old/`. Schemas for generation are loaded from `schemas/Schemas_ESAT.md` only (`load_schemas_esat_markdown` in `project.py`). Legacy trees and one-off scripts sit under `_archive/` (not imported by the pipeline).

## Directory Structure

```
esat_question_generator/
├── project.py             # Main pipeline implementation (run_once, LLM client, parsers)
├── PROJECT_STRUCTURE.md   # This documentation file
├── by_subject_prompts/    # new/Math1 (math); old/ (P/B/C + shared Verifier, Style, Retry, KaTeX_Fixer)
├── schemas/               # Schema markdown (NSAA / ESAT / shared TMUA files)
├── curriculum/            # ESAT_CURRICULUM.json for tag labeling
├── worker_manager.py      # Concurrent generation workers
├── generate_with_progress.py
└── _archive/              # old_prompt_structure, legacy Maths prompts, optional maintenance scripts
```
...
**Designer → Implementer → Verifier → Style Judge → KaTeX validation (and optional format fix) → curriculum classifier (optional) → Save**
