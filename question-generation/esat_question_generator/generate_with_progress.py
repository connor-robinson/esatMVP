...
def main():
    ...
    # Layout: repo/question-generation/esat_question_generator/this_file.py
    ...
    category_order = [
        x.strip() for x in os.environ.get("CATEGORY_ORDER", "M,P,B,C").split(",") if x.strip()
    ]
    ...
    cfg = RunConfig(
        ...
        allow_schema_prefixes=tuple(
            p.strip() for p in os.environ.get("SCHEMA_PREFIXES", "M").split(",") if p.strip()
        ),
    )
