...
# question-generation/ bundles schema_generator + esat_question_generator as siblings
_SCHEMA_GEN_DIR = Path(__file__).resolve().parent
QUESTION_GEN_ROOT = _SCHEMA_GEN_DIR.parent
# Monorepo root (Next app, .env.local)
PROJECT_ROOT_DEFAULT = QUESTION_GEN_ROOT.parent

# Default papers directory (can be overridden via env var or UI)
PAPERS_DIR_DEFAULT = _SCHEMA_GEN_DIR / "papers"

# Central schemas directory (replaces legacy "1. Designer" folder)
SCHEMAS_DIR_DEFAULT = QUESTION_GEN_ROOT / "esat_question_generator" / "schemas"
...
class App(tk.Tk):
    ...
    def on_index_engaa_partb(self):
        ...
        engaa_dir = self.papers_dir / "ENGAA"
