...
# -*- coding: utf-8 -*-
"""
TMUA Question Generation with Progress Tracking
...
from project import safe_load_dotenv, difficulty_weights_from_env
...
def main():
    ...
    # qgen_root = question-generation/; project_root = monorepo root (.env.local)
    ...
    # For TMUA, category_order is M (Paper1), R (Paper2) - these are the schema prefixes
    category_order = os.environ.get("CATEGORY_ORDER", "M,R").split(",")
    category_order = [c.strip() for c in category_order]  # Clean up whitespace

    schema_coverage_path = os.path.join(
        qgen_root, "schema_generator", "_cache", "schema_coverage.json"
    )
    if not os.path.exists(schema_coverage_path):
        schema_coverage_path = os.path.normpath(
            os.path.join(base_dir, "..", "schema_generator", "_cache", "schema_coverage.json")
        )
    ...
    # For TMUA, we need both M (Paper1) and R (Paper2) prefixes
    # Default to M,R for TMUA (override env var if it's set to ESAT defaults like M,P)
    schema_prefixes_env = os.environ.get("SCHEMA_PREFIXES", "M,R")
    # Ensure R is included for TMUA Paper2
    schema_prefixes_list = [p.strip().upper() for p in schema_prefixes_env.split(",")]
    if "R" not in schema_prefixes_list:
        schema_prefixes_list.append("R")
    if "M" not in schema_prefixes_list:
        schema_prefixes_list.append("M")

    # Print startup message
    print("Starting TMUA question generation...", file=sys.stderr)
    ...
    print(f"Schema prefixes: {schema_prefixes_list}", file=sys.stderr)
    ...
    cfg = RunConfig(
        ...
        difficulty_weights=difficulty_weights_from_env(tuple(schema_prefixes_list)),
        schema_weights=None,
        out_dir=os.environ.get("OUT_DIR", "runs"),
        allow_schema_prefixes=tuple(schema_prefixes_list),
    )
    ...
    # Note: category_order is ["M", "R"] for TMUA (M = Paper1, R = Paper2)
