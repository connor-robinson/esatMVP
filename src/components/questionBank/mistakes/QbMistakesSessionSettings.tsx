...
interface QbMistakesSessionSettingsProps {
  summary: QbMistakesSummary | null;
  loadingSummary: boolean;
  starting: boolean;
  error: string | null;
  onStart: (config: QbMistakesLaunchConfig) => void;
}

export function QbMistakesSessionSettings({
  summary,
  loadingSummary,
  starting,
  error,
    ...
    <div className="mx-auto w-full max-w-[960px] rounded-[4px] bg-surface p-8 sm:p-10">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1.5 text-sm text-text-muted">
            Drill question-bank questions you got wrong.
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 space-y-3">
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          {QB_MISTAKES_POOL_OPTIONS.map((option) => {
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 sm:gap-6">
        ...
        <div className="space-y-3">
          ...
          <NumericStepper
            ...
            max={Math.max(
              QUESTION_MIN,
              Math.min(QUESTION_MAX, poolAvailable || QUESTION_MAX),
            )}
            step={1}
            suffix="Qs"
