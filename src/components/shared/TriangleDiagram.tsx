...
export function TriangleDiagram({ data, className }: TriangleDiagramProps) {
  ...
  const sideLabelPositions = useMemo(() => {
    return sides.map((side, index) => {
      ...
      const norm1 =
        len1 > 0 ? { x: perp1.x / len1, y: perp1.y / len1 } : { x: 0, y: 0 };
      const norm2 =
        len2 > 0 ? { x: perp2.x / len2, y: perp2.y / len2 } : { x: 0, y: 0 };
      ...
      const dist1ToCentroid = Math.sqrt(
        (test1X - centroidX) ** 2 + (test1Y - centroidY) ** 2,
      );
      ...
      // Distance from this test point to the centroid
      const dist2ToCentroid = Math.sqrt(
        (test2X - centroidX) ** 2 + (test2Y - centroidY) ** 2,
      );
  ...
  const rightAngleMarkerData = useMemo(() => {
    ...
    const n1 =
      len1 > 0 ? { x: side1.x / len1, y: side1.y / len1 } : { x: 0, y: 0 };
    const n2 =
      len2 > 0 ? { x: side2.x / len2, y: side2.y / len2 } : { x: 0, y: 0 };
    ...
    const corner2 = {
      x: vertex.x + n1.x * markerSize,
      y: vertex.y + n1.y * markerSize,
    };
    const corner3 = {
    ...
    const corner4 = {
      x: vertex.x + n2.x * markerSize,
      y: vertex.y + n2.y * markerSize,
    };
