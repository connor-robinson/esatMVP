...
export function SubjectCategories({
  ...
  return (
    <aside className='scrollbar-hide hidden h-full min-h-0 w-[5rem] shrink-0 flex-col overflow-hidden rounded-organic-xl bg-surface lg:flex xl:w-[5.75rem]'>
      <nav className='flex h-full min-h-0 flex-1 flex-col items-center justify-evenly px-1.5 py-2'>
        {SIDEBAR_CATEGORY_ORDER.map((category) => {
          ...
          return (
            <button
              ...
              className={cn(
                'flex w-full flex-col items-center justify-center gap-1.5 px-1 py-1 transition-all',
                !isSelected && 'opacity-70 hover:opacity-100',
              )}
            >
              <div
                className={cn(
                  'flex h-12 w-12 items-center justify-center rounded-organic-lg transition-colors duration-150 ease-out xl:h-[3.25rem] xl:w-[3.25rem]',
                  isSelected
              ...
              <span
                className={cn(
                  'text-center text-[10px] font-medium leading-tight tracking-[0.06em] xl:text-[11px]',
        ...
        {onLaunchFermiGuessr ? (
          <button
            ...
            className='group flex w-full flex-col items-center justify-center gap-1.5 px-1 py-1 outline-none opacity-90 transition-opacity hover:opacity-100'
            title={`${FERMI_GUESSR_NAME} — daily estimation game`}
          >
            <div className='relative flex h-12 w-12 items-center justify-center rounded-organic-lg bg-surface-elevated xl:h-[3.25rem] xl:w-[3.25rem]'>
            ...
            <span className='text-center text-[10px] font-medium leading-tight tracking-[0.06em] text-secondary xl:text-[11px]'>
              {FERMI_GUESSR_NAME}
            </span>
          </button>
        ) : null}
