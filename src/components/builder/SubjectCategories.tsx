...
export function SubjectCategories({
  ...
  return (
    <aside className='scrollbar-hide hidden h-full min-h-0 w-[4.5rem] shrink-0 flex-col overflow-hidden rounded-organic-xl bg-surface lg:flex xl:w-[5.25rem]'>
      <div className='scrollbar-hide flex min-h-0 flex-1 flex-col items-center overflow-y-auto overflow-x-hidden px-1.5 py-3'>
        <div className='flex w-full flex-col items-center space-y-4'>
          {SIDEBAR_CATEGORY_ORDER.map((category) => {
            ...
            return (
              ...
              >
                <div
                  className={cn(
                    'flex h-10 w-10 items-center justify-center rounded-organic-lg transition-colors duration-150 ease-out xl:h-11 xl:w-11',
                    isSelected
                ...
                >
                  <Icon
                    className={cn(
                      'h-[1.125rem] w-[1.125rem] xl:h-5 xl:w-5',
          ...
          {onLaunchFermiGuessr && (
            <>
              ...
              >
                <div className='relative flex h-10 w-10 items-center justify-center rounded-organic-lg bg-surface-elevated xl:h-11 xl:w-11'>
                  <span className='absolute -right-1 -top-1.5 text-[9px] font-bold uppercase tracking-[0.08em] text-error'>
                    New
                  </span>
                  <FermiGuessrIcon className='h-[1.125rem] w-[1.125rem] text-secondary xl:h-5 xl:w-5' strokeWidth={2} />
                </div>
