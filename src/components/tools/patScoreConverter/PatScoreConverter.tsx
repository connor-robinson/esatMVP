...
import { PatScoreConverterFaq } from "@/components/tools/patScoreConverter/PatScoreConverterFaq";
import { PatHistoricalTable } from "@/components/tools/patScoreConverter/PatHistoricalTable";
      ...
      {open ? (
        ...
        >
          <div className="mb-3 flex items-start justify-between gap-2">
            <p className="text-sm font-semibold text-text">How we estimate PAT percentiles</p>
            <button
              type="button"
          ...
          <div className="space-y-2.5 text-xs leading-relaxed text-text-muted">
            <p>
              We use University of Oxford Physics admissions reports for the PAT
              year you choose. Mean, standard deviation and any PAT-only
              shortlisting mark are official historical benchmarks for that sitting.
            </p>
            <p>
              Percentiles are estimated from that year&apos;s published cohort
              statistics. The ESAT Physics figure is a percentile-equivalent for
              practice context, not an official conversion.
            </p>
          </div>
...
function methodBadgeLabel(result: PatConvertResult): string {
  if (result.percentileApproximate) return "Approximate percentile";
  return percentileMethodLabel(result.percentileMethod);
}

function calculationSteps(result: PatConvertResult): string[] {
  const steps = [
    `We use Oxford's published PAT data for ${result.year}, from ${result.sourceDocumentTitle}.`,
  ];
  if (result.percentile != null && result.percentileApproximate) {
    steps.push(
      `We estimate where your score sat in that year's candidate distribution using a normal-distribution approximation based on Oxford's published mean (${formatStat(result.mean)}) and standard deviation (${formatStat(result.sd)}). This is not an exact percentile.`,
    );
  } else if (result.percentile != null) {
    steps.push(
      "We estimate where your score sat in that year's candidate distribution from Oxford's published score bins, interpolating within a bin when needed.",
    );
  } else {
    steps.push(
      "There is not enough published distribution data to estimate a percentile for this year, so none is shown.",
    );
  }
  if (result.esatPhysicsEquivalent != null) {
    steps.push(
      "We find the ESAT Physics score at the same percentile on the current official ESAT Physics cumulative distribution.",
    );
    steps.push(
      "The ESAT result is only a percentile-equivalent, not an official conversion.",
    );
  }
  return steps;
}
  ...
  return (
    <div className="relative blur-[2px] opacity-50 saturate-50" aria-hidden>
      ...
      >
        ...
        <polygon
          points={areaPoints}
          fill="color-mix(in srgb, var(--color-physics) 12%, transparent)"
        />
        <polyline
          points={linePoints}
          fill="none"
        ...
        <line
          ...
          x2={ghostX}
          y2={h - pad}
          stroke="color-mix(in srgb, var(--color-physics) 25%, transparent)"
          strokeDasharray="4 4"
        />
        <circle
          cx={ghostX}
...
function ResultsPanel({ result }: { result: PatConvertResult }) {
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-4xl font-bold tabular-nums text-physics sm:text-5xl">
      ...
      <dl className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-organic-lg bg-surface-mid px-4 py-3">
          ...
          <dd className="mt-0.5 text-xl font-semibold tabular-nums text-text">
            {result.shortlistingBenchmark != null
              ? result.shortlistingBenchmark
              : "–"}
          </dd>
        </div>
      </dl>

      {result.esatPhysicsEquivalent != null ? (
        <div className="rounded-organic-lg bg-surface-subtle px-4 py-3.5">
          <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-text-muted">
            Rough ESAT Physics percentile-equivalent
          </p>
          <p className="mt-1 text-3xl font-bold tabular-nums text-physics">
            {round1(result.esatPhysicsEquivalent).toFixed(1)}
          </p>
          <p className="mt-1 text-xs leading-relaxed text-text-muted">
            PAT and ESAT are different tests. This is a percentile-equivalent for
            practice context, not an official conversion.
          </p>
        </div>
      ) : null}
      ...
      {result.percentileApproximate ? (
        <NoteRow warning>
          Approximate percentile: this uses a normal-distribution approximation
          based on Oxford&apos;s published mean and standard deviation. It is not
          an exact historical rank.
        </NoteRow>
      ) : null}

      {result.shortlistingNote ? <NoteRow>{result.shortlistingNote}</NoteRow> : null}
...
type Props = {
  esatPhysicsRows: EsatRow[];
};

export function PatScoreConverter({ esatPhysicsRows }: Props) {
  const years = useMemo(() => listPatYearsNewestFirst(), []);
  const [year, setYear] = useState<number | null>(null);
  const [scoreDraft, setScoreDraft] = useState("");
  const [hasCalculated, setHasCalculated] = useState(false);
  const [resultLoading, setResultLoading] = useState(false);
  const [resultError, setResultError] = useState<string | null>(null);
  const [result, setResult] = useState<PatConvertResult | null>(null);
  ...
  const runConvert = () => {
    ...
    if (next.error === "unsupported_year") {
      setResultError("That PAT year is not in the verified Oxford dataset.");
      setResult(null);
      setResultLoading(false);
      return;
    }
    if (next.error === "invalid_score") {
      setResultError("Enter a PAT score between 0 and 100.");
      setResult(null);
      setResultLoading(false);
      return;
    }
    ...
    trackEvent("score_conversion_completed", {
      ...
      converter_page: currentGaPath() ?? "/tools/score-converter/pat",
    });
    setResultLoading(false);
    ...
    <Container id="score-converter" size="lg" className="scroll-mt-24 py-8 sm:py-10">
      <div className="mb-4">
        ...
        <p className="w-full text-sm font-medium leading-snug tracking-tight text-text-muted sm:text-base">
          Enter an Oxford PAT past-paper score to estimate its historical
          percentile and a rough ESAT Physics percentile-equivalent.
        </p>
      </div>
          ...
          <div className="flex flex-wrap items-end gap-3 sm:gap-4">
            ...
            <div className="min-w-[7rem] flex-1">
              <label className="block">
                <span className={fieldLabel}>PAT score</span>
                <div className="flex h-10 items-center gap-2">
                  <input
                    ...
                    value={scoreDraft}
                    placeholder="0"
                    aria-label="PAT score out of 100"
                    onChange={(event) => {
