...
/**
 * Local preview of the full feedback-referral flow. No login required.
 * Open /dev/feedback-referral
 */
export default function DevFeedbackReferralPage() {
  const [stage, setStage] = useState<Stage>("invite");
  const [code, setCode] = useState<string | null>(null);
  const [sharePath, setSharePath] = useState("/pricing?code=CAMP50-PREVIEW");
      ...
      <Container className="relative py-8 sm:py-10">
        <div className="mx-auto max-w-2xl">
          ...
          <p className="mt-2 text-sm text-text-muted">
            {subtitle}. Walk invite → survey → code → friend pricing page. Uses a
            fake preview code; nothing is written to Stripe or the database.
          </p>
          ...
          >
            <FeedbackSurveyForm
              ...
              onComplete={(result) => {
                setCode(result.code);
                try {
                  const url = new URL(result.shareUrl, "http://localhost");
                  setSharePath(`${url.pathname}${url.search}`);
                } catch {
                  setSharePath(`/pricing?code=${encodeURIComponent(result.code)}`);
                }
                setStage("done");
              }}
            />
