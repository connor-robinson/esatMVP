...
export default function FermiStatsPage() {
  ...
  const loginHref = useMemo(
    () => `/login?redirectTo=${encodeURIComponent("/mental-maths/fermi/stats")}`,
    [],
  );
  ...
  const loadStats = useCallback(async (date?: string) => {
    ...
    try {
      ...
      setError("Could not load your Fermi stats. Try again in a moment.");
    } finally {
      setLoading(false);
    }
  }, [loginHref, router]);
    ...
    return (
      <div className="flex min-h-[calc(100vh-65px)] items-center justify-center bg-background px-4">
        <div className="max-w-md text-center">
          ...
          <h1 className="text-2xl font-bold text-text">Fermi stats</h1>
                ...
                <div>
                  <h2 className="text-lg font-bold text-text">
                    {stats.focus.puzzleNumber != null
                      ? `Daily #${stats.focus.puzzleNumber}`
                      : "Today's puzzle"}
                  </h2>
                  <p className="text-sm font-medium text-text-muted">
