  ...
  return (
    <div className="min-h-[calc(100vh-58px)] bg-background py-12">
      <Container size="md">
        ...
        {status.kind === "ready" && !status.completed ? (
          <FeedbackSurveyForm
            onComplete={(result) =>
              setStatus({
                kind: "done",
...
function CodeCard({
  ...
  return (
    <div className="rounded-organic-xl bg-surface-elevated p-6 sm:p-8">
      <h1 className="text-2xl font-bold text-text">Your friend code</h1>
      <p className="mt-2 text-sm text-text-muted">
        One friend can use this for 50% off their first payment. It cannot be
        used on your own account, and it only works once.
      </p>
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <p className="rounded-organic-lg bg-surface-subtle px-4 py-3 font-mono text-lg font-bold tracking-wide text-text">
          {code}
        </p>
        <button
          type="button"
          onClick={() => onCopy(code)}
          className="inline-flex items-center gap-2 rounded-full bg-text px-4 py-2.5 text-sm font-semibold text-background"
        >
      ...
      <button
        ...
        className="mt-2 block w-full truncate rounded-organic-md bg-surface-subtle px-4 py-3 text-left text-sm text-text"
      >
        {shareUrl}
      </button>
      <Link
