...
function AdminReportedQuestionsReviewPage() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(
      `/api/admin/question-reports?status=${encodeURIComponent(statusFilter)}`,
      { cache: "no-store" },
    );
    ...
    setItems(next);
    const ticketIndex = ticketParam
      ? next.findIndex((item) => item.meta.ticketId === ticketParam)
      : -1;
    setIndex(ticketIndex >= 0 ? ticketIndex : 0);
    setEditing(false);
    setLoading(false);
  }, [statusFilter, ticketParam]);
    ...
    return (
      <div className="mx-auto max-w-lg px-6 py-16 text-center">
        ...
        <p className="mt-3 text-sm text-text-muted">
          No reports in this filter. Switch filters on the dashboard, or wait for
          new in-session reports.
        </p>
  ...
  return (
    <div className="relative min-h-screen bg-surface">
      ...
      <QuestionBankEsatSessionShell
        ...
        onOpenLeaveConfirm={() => router.push("/admin/question-reports")}
        onCloseLeaveConfirm={() => {}}
        onSaveAndLeave={() => {}}
        onDiscardSession={() => {}}
        onUseClassicUi={() => {}}
...
export default function AdminQuestionReportsReviewRoute() {
  return (
    <Suspense
      fallback={
        <div className="flex min-h-[50vh] items-center justify-center text-sm text-text-muted">
    ...
    >
      <AdminReportedQuestionsReviewPage />
    </Suspense>
  );
}
