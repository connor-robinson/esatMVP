#!/usr/bin/env python3
"""
MathJax Formatting Validator

Validates that LaTeX math expressions use consistent delimiters
and are compatible with MathJax rendering.
"""
...
def validate_mathjax_formatting(text: str) -> Tuple[bool, List[str]]:
    """
    Validate that text uses consistent MathJax delimiters.
...
def normalize_mathjax_formatting(text: str) -> str:
    """
    Normalize MathJax formatting to ensure consistency.
...
def validate_question_package(question_obj: Dict) -> Tuple[bool, List[str]]:
    """
    Validate MathJax formatting in a complete question package.
    ...
    if isinstance(question, dict):
        ...
        if stem:
            is_valid, errors = validate_mathjax_formatting(stem)
            if not is_valid:
        ...
        if isinstance(options, dict):
            for opt_key, opt_text in options.items():
                if opt_text:
                    is_valid, errors = validate_mathjax_formatting(str(opt_text))
                    if not is_valid:
    ...
    if isinstance(solution, dict):
        ...
        if reasoning:
            is_valid, errors = validate_mathjax_formatting(reasoning)
            if not is_valid:
                all_errors.extend([f"Solution reasoning: {e}" for e in errors])
        
        key_insight = solution.get("key_insight", "")
        if key_insight:
            is_valid, errors = validate_mathjax_formatting(key_insight)
            if not is_valid:
...
def fix_mathjax_formatting(question_obj: Dict) -> Dict:
    """
    Fix MathJax formatting issues in a question package.
    ...
    if isinstance(question, dict):
        ...
        stem = question.get("stem", "")
        if stem:
            question["stem"] = normalize_mathjax_formatting(stem)
        ...
        if isinstance(options, dict):
            fixed_options = {}
            for opt_key, opt_text in options.items():
                fixed_options[opt_key] = normalize_mathjax_formatting(str(opt_text))
    ...
    if isinstance(solution, dict):
        reasoning = solution.get("reasoning", "")
        if reasoning:
            solution["reasoning"] = normalize_mathjax_formatting(reasoning)
        
        key_insight = solution.get("key_insight", "")
        if key_insight:
            solution["key_insight"] = normalize_mathjax_formatting(key_insight)
...
if __name__ == "__main__":
    ...
    is_valid, errors = validate_mathjax_formatting(test_text)
    ...
    normalized = normalize_mathjax_formatting(test_text2)
