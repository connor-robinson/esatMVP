/**
 * PDF of ONLY the Math 2 mock questions changed by apply-math2-replacements.ts.
 *
 *   npx tsx scripts/generate-math2-replacements-review-pdf.ts
 *
 * Output: public/downloads/mocks/maths-2/ESAT CAMP Math 2 Replacements Review.pdf
 * Copies to ~/Downloads as well.
 */
...
const OUT_DIR = path.join(ROOT, "public", "downloads", "mocks", "maths-2");
...
const REVIEW: Array<{
  ...
  { letter: "A", position: 4, action: "REPLACE", reason: "MM7.5 trapezium rule (was trig integration)" },
  { letter: "B", position: 9, action: "REPLACE", reason: "MM4.1 ambiguous sine rule" },
  { letter: "B", position: 23, action: "REPLACE", reason: "MM8.2 transformations (cut S_inf repetition)" },
  { letter: "C", position: 17, action: "INSERT", reason: "fill missing Q17; MM6 stationary-point reasoning" },
  { letter: "C", position: 18, action: "REPLACE", reason: "MM2.3 finite GP (was broken)" },
  { letter: "C", position: 24, action: "REPLACE", reason: "MM7.6 dy/dx = f(x) polynomial DE" },
  { letter: "D", position: 17, action: "REPLACE", reason: "MM2.1 recurrence (was log sum)" },
  { letter: "D", position: 23, action: "REPLACE", reason: "MM8.2 function composition" },
  { letter: "E", position: 1, action: "REPLACE", reason: "MM6.3 normal to a curve (was ln integral)" },
];

const ACTIVE_REVIEW = REVIEW;
const OUT_STEM = "ESAT CAMP Math 2 Replacements Review";
...
<title>Math 2 Replacements Review</title>
<style>${reviewCss(font500, font700)}</style></head><body>
<h1>Math 2 · Replacements review</h1>
<p class="subtitle">Only the nine Math 2 slots replaced or inserted (A4, B9, B23, C17, C18, C24, D17, D23, E1). For checking - not a full paper.</p>
...
function buildKeyHtml(questions: ReviewQ[], font500: string, font700: string): string {
  ...
  return `<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"/>
<title>Math 2 Replacements Review Answer Key</title>
<style>${reviewCss(font500, font700)}</style></head><body>
<h1>Math 2 · Replacements answer key</h1>
