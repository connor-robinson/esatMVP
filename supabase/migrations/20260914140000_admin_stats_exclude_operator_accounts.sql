-- Shared exclusion for admin analytics: seed users, admin-role accounts,
-- and known operator / alt emails (should not skew product stats).
CREATE OR REPLACE FUNCTION public.is_excluded_from_admin_stats(
  p_email text,
...
AS $$
  SELECT
    ...
    OR COALESCE(p_email, '') ILIKE '%@seed.esatcamp.local'
...
REVOKE ALL ON FUNCTION public.is_excluded_from_admin_stats(text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.is_excluded_from_admin_stats(text, text) TO service_role;

COMMENT ON FUNCTION public.is_excluded_from_admin_stats(text, text) IS
  'True when a profile should be omitted from admin dashboards (admins, seeds, operator alts).';

-- Question bank stats: apply shared exclusion.
...
-- Past paper stats: apply shared exclusion.
