-- Split In2scienceUK out of the catch-all "other" cohort segment.
-- Partner slug in production: in2scienceuk (display name In2scienceUK).
...
BEGIN
  ...
  seg AS (
    SELECT u.id AS user_id,
      CASE
        ...
        WHEN EXISTS (
          SELECT 1 FROM partner_users pu
          WHERE pu.user_id = u.id AND pu.slug = 'in2scienceuk'
        ) THEN 'in2scienceuk'
        ELSE 'other'
      END AS segment
    FROM auth.users u
        ...
        WHERE EXISTS (
          ...
          WHERE e.user_id = s.user_id
            ...
            AND (
              ...
              OR (s.segment = 'in2scienceuk' AND p.slug = 'in2scienceuk')
            )
        )
      )::int AS activated,
      (
  ...
  seats AS (
    ...
    FROM partners p
    WHERE p.slug IN ('arkwright-2026', 'elephant26', 'abingdon26', 'in2scienceuk')
  ),
  prefs AS (
