-- Rank hardest questions by each user's first attempt only, so retries
-- cannot inflate attempt counts or wrong rates.
...
BEGIN
  WITH attempts AS (
    ...
    WHERE NOT public.is_excluded_from_admin_stats(p.email, p.role)
  ...
  first_attempts AS (
    SELECT DISTINCT ON (user_id, question_id)
      ...
      at
    FROM attempts
    ORDER BY user_id, question_id, at ASC
  ),
  summary AS (
    SELECT
  ...
  most_wrong_raw AS (
    SELECT
      ...
      COUNT(*)::int AS users,
      COUNT(*) FILTER (WHERE NOT fa.is_correct)::int AS wrong,
      COUNT(*) FILTER (WHERE fa.is_correct)::int AS correct,
      COALESCE(ta.total_attempts, 0)::int AS attempts,
      ROUND(
        100.0 * AVG(CASE WHEN fa.is_correct THEN 1 ELSE 0 END),
        1
      ) AS pct_correct,
      ROUND(
        100.0 * AVG(CASE WHEN NOT fa.is_correct THEN 1 ELSE 0 END),
        1
      ) AS pct_wrong,
      ROUND(
        100.0 * ((COUNT(*) FILTER (WHERE NOT fa.is_correct)) + 2.0)
          / (COUNT(*) + 4.0),
        2
      ) AS plus_four_wrong_pct
    FROM first_attempts fa
    JOIN public.ai_generated_questions q ON q.id = fa.question_id
    LEFT JOIN (
      SELECT question_id, COUNT(*)::int AS total_attempts
      FROM attempts
      GROUP BY question_id
    ) ta ON ta.question_id = fa.question_id
  ...
  most_wrong AS (
    SELECT *
    FROM most_wrong_raw
    ORDER BY plus_four_wrong_pct DESC, wrong DESC, users DESC
    LIMIT GREATEST(p_wrong_limit, 1)
  ),
  option_counts AS (
    SELECT
      fa.question_id,
      fa.user_answer AS option_letter,
      COUNT(*)::int AS count
    FROM first_attempts fa
    WHERE fa.question_id IN (SELECT question_id FROM most_wrong)
      AND fa.user_answer IS NOT NULL
    GROUP BY fa.question_id, fa.user_answer
  ),
  option_totals AS (
  ...
  SELECT jsonb_build_object(
    ...
    'mostWrong', COALESCE(
      (
        SELECT jsonb_agg(
          to_jsonb(m)
          ORDER BY m.plus_four_wrong_pct DESC, m.wrong DESC, m.users DESC
        )
...
BEGIN
  ...
  summary AS (
    SELECT
      ...
      COUNT(DISTINCT user_id)::int AS unique_users,
      COALESCE(SUM(NULLIF((score->>'correct')::numeric, 0)), 0)::numeric AS score_correct_sum,
      COALESCE(SUM(NULLIF((score->>'total')::numeric, 0)), 0)::numeric AS score_total_sum,
      COALESCE(
        ROUND(
          AVG(
            CASE
  ...
  most_wrong_raw AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE fa.is_correct)::int AS correct,
      COALESCE(ta.total_attempts, 0)::int AS attempts,
      ROUND(100.0 * AVG(CASE WHEN fa.is_correct THEN 1 ELSE 0 END), 1) AS pct_correct,
      ROUND(100.0 * AVG(CASE WHEN NOT fa.is_correct THEN 1 ELSE 0 END), 1) AS pct_wrong,
      ROUND(
        100.0 * ((COUNT(*) FILTER (WHERE NOT fa.is_correct)) + 2.0)
          / (COUNT(*) + 4.0),
        2
  ...
  option_counts AS (
    SELECT
      fa.question_id,
      fa.choice AS option_letter,
      COUNT(*)::int AS count
    FROM first_attempts fa
    WHERE fa.question_id IN (SELECT question_id FROM most_wrong)
    GROUP BY fa.question_id, fa.choice
  ),
  option_totals AS (
    SELECT question_id, SUM(count)::int AS answered
    FROM option_counts
  ...
  SELECT jsonb_build_object(
    ...
    'ranking', 'plus_four_wrong_first_attempt',
    'summary', COALESCE(
      (SELECT to_jsonb(s) || to_jsonb(a) FROM summary s, answer_summary a),
      '{}'::jsonb
    ),
    'papers', COALESCE(
      (SELECT jsonb_agg(to_jsonb(p) ORDER BY p.sessions DESC, p.users DESC) FROM by_paper p),
      '[]'::jsonb
    ),
    'sections', COALESCE(
      (SELECT jsonb_agg(to_jsonb(s) ORDER BY s.sessions DESC, s.users DESC) FROM by_section s),
      '[]'::jsonb
    ),
    'mostWrong', COALESCE(
      (
